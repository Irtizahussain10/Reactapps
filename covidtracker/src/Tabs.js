...
export default function ScrollableTabsButtonAuto() {
    ...
    useEffect(() => {
        async function gettingCountryNames() {
    ...
    return (
        <div className={classes.root}>
            <AppBar position="static" color="default">
                ...
                >
                    {globalData.map((key, item) => {
                        return <Tab label={key} {...a11yProps(item)}></Tab>
                    })}
                </Tabs>
            </AppBar>
            <div>
                {globalData.map((key, item) => {
                    return <TabPanel value={value} index={item}>Hi {key}</TabPanel>
                })}
            </div>
        </div>
    );
